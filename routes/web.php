...
Route::middleware(['auth', 'request.context.authenticated', 'verified', 'account.active'])->group(function (): void {
    ...
    Route::get('/products', [ProductDemoController::class, 'index'])->middleware('can:products.view')->name('products.index');
    Route::get('/products/create', [ProductDemoController::class, 'create'])->middleware('can:products.create')->name('products.create');
    Route::post('/products', [ProductDemoController::class, 'store'])->middleware('can:products.create')->name('products.store');
    Route::get('/products/{productId}', [ProductDemoController::class, 'show'])->whereNumber('productId')->middleware('can:products.view')->name('products.show');
    Route::get('/products/{productId}/edit', [ProductDemoController::class, 'edit'])->whereNumber('productId')->middleware('can:products.update')->name('products.edit');
    Route::put('/products/{productId}', [ProductDemoController::class, 'update'])->whereNumber('productId')->middleware('can:products.update')->name('products.update');
    Route::delete('/products/{productId}', [ProductDemoController::class, 'destroy'])->whereNumber('productId')->middleware('can:products.delete')->name('products.destroy');
