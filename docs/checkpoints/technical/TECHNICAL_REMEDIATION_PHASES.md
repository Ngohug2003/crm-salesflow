...
| TR-01 | Test và quality checkpoint toàn hệ thống | `feature/tr-01-system-quality-checkpoint` | P1/P2/P3 technical features | Hoàn tất triển khai — chờ kiểm thử | Toàn bộ test/quality/build đạt |
| TR-02 | Khôi phục và đồng bộ tài liệu | `docs/tr-02-requirements-sync` | TR-01 | Hoàn tất triển khai — chờ kiểm thử | Requirement, skill, README và docs đúng code |
| TR-03 | Tách và chuẩn hóa phase log | `docs/tr-03-phase-log-split` | TR-02 | Hoàn tất triển khai — chờ kiểm thử | Roadmap gọn, lịch sử vẫn được giữ và liên kết |
...
**Ngày**: 23/07/2026
**Branch**: `feature/p3-t02-lead-duplicate-guard`
**Requirement**: `REQ-7.2`, `GAP-DUP-001`

**Files thay đổi**:
- `app/Exceptions/DuplicateLeadException.php` [NEW] — Định nghĩa exception trùng lặp lưu trữ danh sách ứng viên trùng và chữ ký liên hệ.
- `app/Services/LeadManagementService.php` — Đưa duplicate detection preflight check xuống tầng Service. Tự động kiểm tra trùng lặp và ném `DuplicateLeadException` nếu thiếu thông tin xác nhận khớp hoặc thiếu lý do ghi đè (hoặc lý do < 10 ký tự). Khi ghi đè thành công, đính kèm `duplicate_override` metadata (gồm danh sách candidate IDs và lý do) vào audit log.
- `app/Livewire/Leads/LeadEditor.php` — Thêm thuộc tính `duplicateOverrideReason`, bọc phương thức `save` trong try/catch bắt `DuplicateLeadException` và cập nhật luồng xác nhận/quay lại chỉnh sửa. Bổ sung validate lý do ghi đè tối thiểu 10 ký tự trước khi xác nhận.
- `resources/views/livewire/leads/lead-editor.blade.php` — Bổ sung trường nhập lý do trùng lặp vào modal và hiển thị lỗi validation.
- `tests/Feature/LeadDuplicateLifecycleTest.php` — Bổ sung test kiểm thử cưỡng chế trùng lặp tại Service, test validate lý do ghi đè ngắn/rỗng, và kiểm thử audit log chứa dữ liệu override.

**Quality gates**:
- `composer quality`: ✅ PASS (170 tests passed, static analysis OK)

Checkpoint P3-T02: **dừng tại đây để kiểm thử create/edit duplicate trước trash conflict**.
...
**Ngày**: 23/07/2026
**Branch**: `feature/tr-01-system-quality-checkpoint`
**Requirement**: `TR-01`

**Các bước đã thực thi và kết quả**:
1. **Kiểm thử tự động**:
   - Lệnh: `docker compose exec app php artisan test`
   - Kết quả: ✅ **172 tests passed (1124 assertions)**
2. **Kiểm tra định dạng code**:
   - Lệnh: `docker compose exec app ./vendor/bin/pint --test`
   - Kết quả: ✅ **PASS** (183 files check, no style issues)
3. **Phân tích tĩnh**:
   - Lệnh: `docker compose exec app ./vendor/bin/phpstan analyse --memory-limit=512M --no-progress`
   - Kết quả: ✅ **[OK] No errors**
4. **Vite assets build**:
   - Lệnh: `docker compose exec vite npm run build`
   - Kết quả: ✅ **built in 3.05s** (app.css 260.22 kB, app.js 74.10 kB, manifest.json 0.33 kB)
5. **Trạng thái migration**:
   - Lệnh: `docker compose exec app php artisan migrate:status`
   - Kết quả: ✅ **All ran** (17 migrations executed successfully)
6. **Docker services**:
   - Lệnh: `docker compose ps`
   - Kết quả: ✅ **All 10 services healthy** (app, horizon, mailpit, minio, nginx, postgres, redis, reverb, scheduler, vite)

Checkpoint TR-01: **dừng tại đây để chủ dự án xác nhận quality baseline trước khi chỉnh tài liệu lớn**.
...
**Ngày**: 23/07/2026
**Branch**: `feature/tr-01-system-quality-checkpoint` (theo yêu cầu gộp của chủ dự án)
**Requirement**: `TR-02`

**Files thay đổi**:
- `README.md` — Cập nhật mục `Current checkpoint` làm rõ việc hoàn tất Phase 3 (Lead lifecycle, duplicate guard, trash/restore conflict) và chốt chất lượng TR-01.
- `docs/requirements.md` — Đồng bộ hóa các quyết định kiến trúc: bổ sung `DEC-009` (lý do thiết kế full-page form cho Lead tạo/sửa thay vì modal) và `DEC-010` (nguyên tắc không cài đặt package bên ngoài bừa bãi).
- `docs/architecture.md` — Chỉnh sửa và làm sạch các đường dẫn liên kết tương đối bị hỏng để đảm bảo các tài liệu kỹ thuật liên kết chính xác với nhau.

**Quality gates**:
- `composer quality`: ✅ PASS (172 tests passed, static analysis OK)

Checkpoint TR-02: **dừng tại đây để chủ dự án đọc requirement/docs trước khi tách phase log**.
...
1. **Lưu trữ các tệp tin chi tiết**:
   - `docs/checkpoints/P0/ANALYSIS.md` — Lưu trữ nhật ký Giai đoạn 0.
   - `docs/checkpoints/P1/PHASE_LOG.md` — Lưu trữ nhật ký Giai đoạn 1.
   - `docs/checkpoints/P2/PHASE_LOG.md` — Lưu trữ nhật ký Giai đoạn 2.
   - `docs/checkpoints/P3/PHASE_LOG.md` — Lưu trữ nhật ký Giai đoạn 3.
   - `docs/checkpoints/technical/TECHNICAL_REMEDIATION_PHASES.md` — Bản sao lưu trữ toàn bộ các thay đổi remediation kỹ thuật.
2. **Chuẩn hóa lộ trình chính**:
   - `PROJECT_PHASES.md` — Dọn dẹp hơn 1500 dòng nhật ký chi tiết, chỉ giữ lại các bảng roadmap gọn gàng hiển thị mã feature, branch, phụ thuộc, trạng thái và các liên kết trỏ tới các tệp tin lưu trữ nhật ký kiểm thử tương ứng.

**Quality gates**:
- `composer quality`: ✅ PASS (172 tests passed, static analysis OK)

Checkpoint TR-03: **dừng tại đây để nghiệm thu toàn bộ technical remediation trước P3-09**.
